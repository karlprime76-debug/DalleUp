...
-- 3. Ajouter les valeurs manquantes à l'enum DriverStatus
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_enum
    WHERE enumlabel = 'PENDING'
    AND enumtypid = (SELECT oid FROM pg_type WHERE typname = 'DriverStatus')
  ) THEN
...
-- 4. Ajouter les colonnes manquantes à la table User
...
-- 5. Définir driverStatus avec une valeur par défaut et rendre NOT NULL
...
-- 6. Marquer la migration 002 comme appliquée
INSERT INTO "_prisma_migrations" ("id", "checksum", "finished_at", "migration_name", "started_at", "applied_steps_count")
