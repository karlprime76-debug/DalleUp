...
export default function RestaurantPayoutAccountPage() {
  ...
  useEffect(() => {
    fetch("/api/restaurant/payout-account")
      .then((r) => r.json())
      .then((d) => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    setMessage(null);
    const res = await fetch("/api/restaurant/payout-account", {
      method: "POST",
  ...
  return (
    <div className="mx-auto max-w-xl p-6">
      ...
      {account?.isVerified && <div className="mb-4 rounded-md border bg-green-50 px-4 py-3 text-sm text-green-800">Compte vérifié. Vous pouvez recevoir vos reversements.</div>}
