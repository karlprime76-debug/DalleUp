...
const nav = [{ href: "/admin", label: "Accueil" }, { href: "/admin/orders", label: "Commandes" }, { href: "/admin/settings", label: "Paramètres" }];
...
export default async function AdminOrdersPage() {
  ...
  return <DashboardShell title="Admin Commandes" nav={nav}><div className="grid gap-4 md:grid-cols-4"><StatCard label="Commandes" value={String(stats.orders)} /><StatCard label="Restaurants" value={String(restaurants.length)} /><StatCard label="Livreurs" value={String(drivers.length)} /><StatCard label="CA" value={formatPrice(stats.revenue)} /></div><Card className="mt-6 p-5"><h2 className="text-xl font-black">Commandes plateforme</h2><div className="mt-4 grid gap-3">{orders.map((order) => <div key={order.id} className="grid gap-3 rounded-2xl bg-neutral-50 p-4 2xl:grid-cols-[120px_1fr_150px_120px_1.1fr_1.2fr]"><b>{order.id}</b><span>{order.restaurant} · {order.customer}<br /><span className="text-xs text-neutral-500">Livreur : {order.driver}</span></span><div className="grid gap-2"><OrderStatusBadge status={order.status} />{order.deliveryStatus ? <Badge variant="neutral">Livraison {order.deliveryStatus}</Badge> : null}</div><span className="font-black text-dalle-orange">{formatPrice(order.total)}</span><OrderStatusActions orderId={order.dbId} role="admin" /><AssignDriverActions orderId={order.dbId} drivers={drivers} currentDriverId={order.driverId} /></div>)}</div></Card></DashboardShell>;
}
