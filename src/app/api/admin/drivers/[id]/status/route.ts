...
const allowedStatuses = ["AVAILABLE", "OFFLINE", "ON_DELIVERY", "SUSPENDED"] as const;
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const updated = await prisma.user.update({ where: { id: driver.id }, data: { driverStatus: status as typeof allowedStatuses[number] } });
