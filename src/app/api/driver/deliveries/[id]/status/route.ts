...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [updatedDelivery] = await prisma.$transaction([
      prisma.delivery.update({ where: { id: delivery.id }, data: updateData }),
      prisma.order.update({ where: { id: delivery.orderId }, data: { status: status === "DELIVERED" ? "DELIVERED" : status as typeof allowedDriverStatuses[number] } }),
      status === "DELIVERED"
        ? prisma.user.update({ where: { id: session.user.id }, data: { driverStatus: "AVAILABLE" } })
        : prisma.user.update({ where: { id: session.user.id }, data: {} })
    ]);
