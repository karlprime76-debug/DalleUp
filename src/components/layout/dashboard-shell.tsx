  ...
  return (
    <div className="min-h-screen bg-neutral-50">
      ...
      <div className="sticky top-0 z-30 flex items-center gap-3 border-b border-black/5 bg-white/90 px-4 py-3 backdrop-blur-xl lg:hidden">
        <button type="button" onClick={() => setMobileOpen(true)} aria-label="Ouvrir le menu" className="grid h-10 w-10 place-items-center rounded-2xl bg-neutral-100 transition hover:bg-neutral-200">
      ...
      {mobileOpen ? (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <div className="absolute left-0 top-0 h-full w-72 bg-dalle-charcoal p-5 text-white shadow-2xl">
            <div className="flex items-center justify-between">
              <Link href={logoHref} onClick={() => setMobileOpen(false)} className="text-xl font-black text-dalle-orange">{site.name}</Link>
              <button type="button" onClick={() => setMobileOpen(false)} aria-label="Fermer le menu" className="grid h-10 w-10 place-items-center rounded-2xl bg-white/10 transition hover:bg-white/20">
                <X size={20} />
              </button>
            </div>
