...
const roleContent = {
  CLIENT: { eyebrow: "Créer un compte client", title: "Rejoins DalleUp", cta: "Créer mon compte" },
  RESTAURANT: { eyebrow: "Créer un compte restaurant", title: "Deviens restaurant partenaire", cta: "Devenir restaurant partenaire" },
  DELIVERY_DRIVER: { eyebrow: "Créer un compte livreur", title: "Deviens livreur DalleUp", cta: "Devenir livreur DalleUp" }
};
...
export function RegisterForm({ role = "CLIENT" }: { role?: "CLIENT" | "RESTAURANT" | "DELIVERY_DRIVER" }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-md rounded-[2rem] bg-white p-6 shadow-sm">
      ...
      <div className="mt-6 grid gap-2 rounded-3xl bg-dalle-cream p-4 text-sm text-neutral-600"><p className="font-black text-dalle-charcoal">Tu es pro ?</p><Link href="/register?role=RESTAURANT" className="font-black text-dalle-orange">Devenir restaurant partenaire</Link><Link href="/register?role=DELIVERY_DRIVER" className="font-black text-dalle-orange">Devenir livreur</Link></div>
    </form>
  );
}
