...
export function RestaurantWalletClient() {
  ...
  async function requestPayout() {
    ...
    setPayoutMsg("");
    try {
      const res = await fetch("/api/restaurant/payout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ amount: data.balance }) });
  ...
  return (
    <div className="mx-auto max-w-3xl p-6">
      <h1 className="mb-6 text-2xl font-bold">Mon solde</h1>
            ...
            <tbody>
              {data.entries.map((entry) => (
                <tr key={entry.id} className="border-b last:border-0">
                  ...
                  <td className="p-3">{entry.type === "RESTAURANT_PAYOUT" ? "Commission restaurant" : entry.type}</td>
